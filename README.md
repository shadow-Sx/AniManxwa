...
## 🆕 Yangiliklar (tez yuklanish, PDF yuklab olish, teglar)

**Yangi paketlar** (`package.json`da qo'shildi, `npm install` bilan o'rnatiladi): `pdfkit` (PDF yasash), `node-fetch` (rasmlarni PDF uchun olib kelish). **Yangi environment o'zgaruvchi kerak emas.**

| Fayl | Holati |
|---|---|
| `config/cloudinary.js` | ✏️ `buildSliceUrls` qo'shildi |
| `models/Chapter.js` | ✏️ sahifalarga `width, height, slices` qo'shildi |
| `models/Series.js` | ✏️ `tags` maydoni qo'shildi |
| `models/User.js` | ✏️ `isSubscribed`, `downloadBonusRemaining` qo'shildi |
| `routes/chapters.js` | ✏️ yuklashda avtomatik kesish |
| `routes/downloads.js` | 🆕 yangi (`POST /api/downloads/chapters/:id`) |
| `routes/admin.js` | ✏️ foydalanuvchilar ro'yxati + obuna berish |
| `routes/auth.js` | ✏️ javobga obuna/bonus ma'lumoti qo'shildi |
| `middleware/userAuth.js` | 🆕 yangi (oddiy login tekshiruvi, admin kodidan alohida) |

### 1) Tez yuklanadigan manhwa sahifalari
Manhwa/manhua turidagi mangaga bob qo'shilganda, har bir uzun sahifa avtomatik ~100 qismga bo'linadi — lekin **hech qanday qo'shimcha fayl saqlanmaydi**. Buning o'rniga Cloudinary'ning o'zi, so'ralganda, bitta asl rasmdan kerakli qismini "kesib" beradi (`c_crop` transformatsiyasi orqali) va keshlaydi. Frontend endi har bir qismni alohida, ekranga yetib kelganda yuklaydi (`loading="lazy"`) — shuning uchun sahifa ancha tezroq ochiladi. Manga/roman turlarida bu qo'llanilmaydi (ular sahifama-sahifa o'qiladi, uzun tasma emas).

### 2) PDF qilib yuklab olish
O'qish oynasi tepasida **"Yuklab olish"** tugmasi — faqat **Bepul** mangalarda ko'rinadi (obuna talab qiladiganlarda umuman yo'q). Bosilganda:
- Tizimga kirmagan bo'lsa → Google orqali kirish so'raladi
- Kirgan bo'lsa → bob barcha sahifalari bitta PDF qilib, to'g'ridan-to'g'ri qurilmaga yuklanadi
- Har bir **yangi** hisob avtomatik **10 ta bepul yuklab olish huquqi** bilan boshlanadi
- 10 tasi tugagach, obuna sotib olgan foydalanuvchilar cheklovsiz yuklab olishda davom etadi; boshqalar "Obuna" bo'limiga yo'naltiriladi
- Admin panelida yangi **"Foydalanuvchilar"** bo'limi qo'shildi — shu yerdan har bir kishiga qo'lda obuna berish/olib tashlash mumkin (haqiqiy to'lov tizimi ulanmaguncha)

### 3) Kirish (login) faqat kerak bo'lganda so'raladi
Bepul mangalarni o'qish — hech qachon login talab qilmaydi. Login faqat quyidagilar uchun so'raladi: yuklab olish, shikoyat yuborish, tarjimonlikka so'rov, va **obuna talab qiladigan** mangani o'qish. Obunasi yo'q (yoki hali obuna sotib olmagan) foydalanuvchi pullik mangani ochishga urinsa, avtomatik "Obuna olish" bo'limiga o'tkaziladi.

### 4) Teglar
Manga qo'shish/tahrirlash formasida endi **"Teglar"** maydoni bor (vergul bilan ajratilgan so'zlar, masalan personaj ismlari). Qidiruv endi nomi bilan bir qatorda teglarni ham tekshiradi.

## 🔍 Google orqali qidirilganda chiqadimi?

Ochig'ini aytsam — **hozircha yo'q, yaxshi chiqmaydi**, va buning texnik sababi bor: bu ilova "client-side" React sahifasi, ya'ni Google'ning qidiruv roboti sahifangizga kirganda dastlab deyarli bo'sh HTML ko'radi (mazmun JavaScript orqali keyin chiziladi). Google buni qisman "o'qiy oladi", lekin sekin va ishonchsiz, va boshqa ko'plab robotlar (masalan ijtimoiy tarmoqlarda havola ulashganda ko'rinadigan preview botlari) buni umuman o'qiy olmaydi.

Kichik yordam sifatida, manga sahifasi ochilganda endi sahifa sarlavhasi (`<title>`) va tavsifi avtomatik o'sha manganing nomiga/tavsifiga almashadi — bu brauzer yorlig'i va ba'zi botlar uchun foydali, lekin **to'liq yechim emas**.

**To'liq yechim** — bu server-side rendering (masalan Next.js'ga o'tish), bu esa butunlay boshqa, ancha katta loyiha bo'ladi. Hozircha buni qurmadim, chunki bu alohida katta ish — xohlasangiz, buni ham keyingi qadam sifatida qo'lga olishimiz mumkin.
